...
*/
@Component
public class PremierLeagueItemReader implements ItemReader<LeagueTable>
{
    private Logger LOG = LogManager.getLogger(PremierLeagueItemReader.class);
     ...
     */
    @Override
    public LeagueTable read () throws Exception, UnexpectedInputException, ParseException, NonTransientResourceException
    {

        LOG.info("Inside the read() of the FirstItemReader class....");
        if(leagueTableDtos == null){
            LOG.info("Initializing the league tables...");
            initialize();
        }

        if(i < leagueTableDtos.size()){
            LeagueTable data = leagueTableDtos.get(i++);
            LOG.info("Getting the data in the read() for team = "+data.getTeamname()+" and the index = "+i);
            return data;
        }
    ...
    private void initialize(){
        ...
        try
        {
            LOG.info("Fetching the details from the rest api..");
            url = new URL(URL);
            HttpURLConnection conn = (HttpURLConnection) url.openConnection();
            conn.setRequestMethod("GET");
            conn.connect();
            byte[] bytes = url.openStream().readAllBytes();
            LOG.info("Response is read by readAll bytes");

            // Step 3: Convert the bytes to a String
            String jsonArray = new String(bytes, StandardCharsets.UTF_8);


            LOG.info("The response converted into a json array string is "+jsonArray);
            ObjectMapper objectMapper = new ObjectMapper();
            leagueTableDtos = objectMapper.readValue(jsonArray, new TypeReference<List<LeagueTable>>() {});
            LOG.info("League table dtos are populated");

        }
        catch (MalformedURLException e)
        {
            LOG.error("Some abnormality occurred during Item Reader Executions", e);
            throw new RuntimeException("Some abnormality occurred during Item Reader Executions due to "+e.getMessage());
        }
